...
(async function init() {
  try {
    ...
    showLoadError(
      noToken
        ? "Belum login — belum ada access_token di localStorage. Login dulu lewat /auth/login, atau set manual: localStorage.setItem('access_token', '...')."
        : `Gagal fetch dari ${API_BASE} — cek backend jalan (uvicorn) dan API_BASE cocok sama host backend-nya. (${e.message})`
    );
  }
})();
